...
if [ ! -f ${dbfile} ]; then
 ...
 ${cmd_sqlite3} ${dbfile} "CREATE TABLE sens_data (id INTEGER PRIMARY KEY AUTOINCREMENT, device VARCHAR(64), sensId INTEGER, current REAL, voltage REAL, timestamp TEXT, updated_at TEXT DEFAULT (datetime(CURRENT_TIMESTAMP, 'localtime')));"
#else
#  echo "${dbfile} exists!"
fi
...
#insert sensor data
    if [ $flag -eq 1 ]; then
        ${cmd_sqlite3} ${dbfile} "INSERT INTO sens_data(device, sensId, current, voltage, timestamp) VALUES('${device}', '${sensId}', '${current}', '${voltage}', '${timestamp}');"
    fi
done < $file